...
def main():
    ...
    outcsv = path.join(args.outdir, path.basename(args.infile) + ".csv")
    if args.annotate:
        outfile = path.join(
                ...
                if ret:
                    ...
                    for box in boxes:
                        x1, y1, x2, y2, score = (
                            ...
                            round(box[3]), box[4])
                        line += "\t" + "\t".join(
                            list(map(str, [x1, y1, x2, y2, score])))

                        if args.annotate and score >= args.score_threshold:
                            cv.rectangle(
                                frame, (x1, y1), (x2, y2), (255, 0, 0))
                            img = Image.fromarray(frame)
                            draw = ImageDraw.Draw(img)
                            font = ImageFont.truetype(
                                path.join(getScriptPath(),
